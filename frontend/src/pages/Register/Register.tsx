        ...
        >
          ...
          <TextField
            ...
            onChange={({ target: { value } }) => {
              // username validation
              if (value == '') setUsernameErrorMsg('');
              // reset error msg if blank
              else if (!/^[a-z0-9_-]+$/i.test(value))
                setUsernameErrorMsg('Username should be alphanumeric');
          ...
          <TextField
            ...
            onChange={({ target: { value } }) => {
              // email validation
              if (value == '') setEmailErrorMsg('');
              // reset error msg if blank
              else if (!/^[a-zA-Z0-9]+@[a-zA-Z0-9]+\.[A-Za-z]+$/.test(value))
                setEmailErrorMsg('Input correct email');
          ...
          <TextField
            ...
            onChange={({ target: { value } }) => {
              // password validation
              if (value == '') setPassErrorMsg('');
              // reset error msg if blank
              else if (value.length < 8)
                setPassErrorMsg('Password is too short');
              else if (value == value.toUpperCase())
                setPassErrorMsg('Input at least one small letter');
              else if (value == value.toLowerCase())
                setPassErrorMsg('Input at least one big letter');
          ...
          <TextField
            ...
            onChange={({ target: { value } }) => {
              ...
              if (value == '') setRepeatPassErrorMsg('');
              // reset error msg if blank
              else if (value != typedPassword)
                setRepeatPassErrorMsg("Passwords don't match");
